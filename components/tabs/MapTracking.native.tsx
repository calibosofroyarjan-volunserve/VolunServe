...
export default function MapTracking() {
      ...
      incidents.find(
        (incident) =>
          (incident.assignedVolunteerId === CURRENT_USER_ID ||
            incident.assignedVolunteerIds?.includes(CURRENT_USER_ID)) &&
          !["resolved", "closed"].includes(incident.status ?? "reported")
      ) ?? null
    );
  ...
  const prediction = useMemo<PredictionResult>(() => {
    ...
    const recommendation =
      riskLevel === "Critical"
        ...
        : riskLevel === "High"
        ? "High operational pressure. Keep volunteers moving, monitor center occupancy, and escalate unresolved high-severity incidents."
        : riskLevel === "Moderate"
  ...
  async function initialize() {
    try {
      ...
      const subscriptions = [
        subscribeToIncidents(resolvedRole),
        subscribeToCenters(),
      ];
  ...
  function subscribeToIncidents(role: "resident" | "volunteer" | "admin") {
    ...
    return onSnapshot(
        ...
        const data: Incident[] = snapshot.docs
          .map((docSnap) => {
            ...
            return {
              ...
              type: raw.category ?? raw.type ?? "emergency",
              severity: raw.severity ?? "medium",
              status: raw.status ?? "reported",
              reportedBy: raw.reporterUid ?? "",
              assignedVolunteerIds: Array.isArray(raw.assignedVolunteerIds)
                ? raw.assignedVolunteerIds
                : [],
      ...
      (error) => {
        console.log("Disaster case map subscription error:", error);
        setLocationError("The response cases could not be loaded from Firebase.");
      }
    );
  }
  ...
  function openFormalDisasterReport() {
    router.push("/disaster-response");
  }

  function openVolunteerAssignments() {
              ...
              >
                ...
                <View style={styles.riskCard}>
                  <View style={styles.riskTopRow}>
                    <Text style={styles.riskLabel}>Operational Risk Estimate</Text>
                ...
                <View style={styles.controlButtonRow}>
                  ...
                  >
                    ...
                    <Text style={styles.secondaryControlText}>
                      {mode === "map" ? "Explore Map Pin" : "Normal Map"}
                    </Text>
                  </TouchableOpacity>
                </View>
                ...
                {userRole === "volunteer" && (
                    <TouchableOpacity
                      activeOpacity={0.82}
                      onPress={openVolunteerAssignments}
